...
const Speaker = ({ speakerData, index }) => {
  ...
  return (
    <div className={`flex-shrink justify-around bg-gray-50 md:bg-none w-80 pt-6 sm:pt-8 md:pt-0 sm:w-96 md:w-full max-w-screen-md mx-auto items-center md:items-stretch flex flex-col ${cn({ "md:flex-row-reverse": index % 2 !== 0, "md:flex-row": index % 2 === 0 })}`}>
      ...
      <div className="px-4 flex-col text-orange-800 w-full md:w-96 mb-8 md:mb-0">
        <p className="mt-4 text-2xl md:text-4xl font-bold ">{title}</p>
        <p className="font-play text-base sm:text-xl md:text-2xl mt-2 text-orange-700">{name}</p>
        <p className="font-play mt-6 text-base sm:text-lg md:text-xl tracking-wide leading-relaxed md:leading-loose">{main}</p>
